...
def return_Y_Yx_fit(YSZ_500,YSZ_500_err,Yx,Yx_err,redshift,mbias=[],qcut=6.,min_type="ORTH",nwalkers=50,nsamples=2000,burnin=500,use_approx_err=False,ana_corr=False,verbose=False,A_prior=[-6.,6],alpha_prior=[-2,2.],int_scat_prior=[-2.,2.]):
	np.random.seed(0)
...
def return_emcee_fit(xdata,ydata,xerr,yerr,A_prior,alpha_prior,int_scat_prior,min_type="SLS",nwalkers=50,nsamples=2000,burnin=500):
	...
	ndim=3
	pos = [[1., -3., 0.] + [0.3,0.5,0.03]*np.random.randn(ndim) for i in range(nwalkers)]
...
def gen_corner_plot(samples,labels=[r"$\alpha$", "$A$", r"$\sigma_{{\rm Log}Y|Yx}$"],figttl="",figname="",numdec=3):
	...
	fig=corner.corner(samples, labels=[r"$\alpha$", "$A$", r"$\sigma_{{\rm Log}Y|Yx}$"])
	...
	fig.text(0.71,0.54,r"$\sigma_{{\rm Log}Y|Yx}=$" + str(round(param[2],numdec)) + "$\pm$" + str(round(param_err[2],numdec)))
	fig.text(0.71,0.51,r"$ \rm{Corr}(\alpha,A)=$" + str(round(corr_mat[0,1],numdec)))
	fig.text(0.71,0.48,r"$ \rm{Corr}(\alpha,\sigma_{{\rm Log}Y|Yx})=$" + str(round(corr_mat[0,2],numdec)))
	fig.text(0.71,0.45,r"$ \rm{Corr}(A,\sigma_{{\rm Log}Y|Yx})=$" + str(round(corr_mat[1,2],numdec)))
	fig.text(0.53,0.86,r"${\rm Log}_{10}\left( D^2_A  Y^{\rm SZ}_{500}\right) = A + \alpha {\rm Log}_{10}\left( \frac{Y^{\rm X}_{500}}{10^{-4}}\right)$",fontsize=12)
	if figname!="":
