...
def extract_data_tiles(do_data=True,do_mask=True,gen_mask=True,verbose=False,glat_thr=0.):
	...
	for pix in pixnum:
		projop=tpa.tangent_plane_setup(mmfset.nside,mmfset.xsize,cglat[pix],cglon[pix],rescale=1)
	...
	if do_data:
		for ich,ch in enumerate(mmfset.planck_channels):
			...
			for pix in pixnum[(fsky>0.3) & (abs(cglat)>glat_thr)]:
				filename=mmfset.paths["tplanes"] + "tile_G" + str(round(cglon[pix],2)) + ["", "+"][cglat[pix] >= 0] + str(round(cglat[pix],2)) + ".fits"
				projop=tpa.tangent_plane_setup(mmfset.nside,mmfset.xsize,cglat[pix],cglon[pix],rescale=1.)
