...
class multi_matched_filter(object):
	...
	def get_data_ft_old(self,data,smwin=5):
	...
	def get_data_ft(self,data,smwin=5,emask=[],mfcm=[]):
		...
		self.cross_Pk=np.zeros((self.totnpix,self.numch,self.numch),np.float64)
		if emask==[]:
			emask=np.ones((self.nxpix,self.nxpix),np.float64)
			bin_mask=emask
		else:
	...
	def return_snr_lk(self,thetac,Tc,maskthr=3.,mask_fdata=True,psmask=[],write_data=False,filename=[]):
		if thetac in self.sp_ft_bank.keys():
			fdata,err=self.evaluate_mmf(self.sp_ft_bank[thetac],self.sz_spec_bank[Tc])
		else:
			# Additional parameters of the cluster profile assumed to be defaults.
			template=sc.gen_cluster_template(gset.mmfset.npix,thetac,pixel_size=gset.mmfset.reso,y0=1.)
			template_alm=fsa.map2alm(np.fft.fftshift(template),gset.mmfset.reso)
			fdata,err=self.evaluate_mmf(template_alm,self.sz_spec_bank[Tc])
		...
		yc=fdata[np.int(gset.mmfset.npix/2),np.int(gset.mmfset.npix/2)]
		snr=yc/err
	...
	def return_optimal_theta500_lk(self,Tc,maskthr=3.,mask_fdata=True,write_data=False,filename=[]):
		ans=np.zeros((4,np.size(self.sp_ft_bank.keys())),float)
		#fdata=np.zeros((np.size(template_ft_bank.keys()),gset.mmfset.npix,gset.mmfset.npix),float)
		theta500=sorted(self.sp_ft_bank.keys())
		for idx,thetac in enumerate(theta500):
			template_ft=self.sp_ft_bank[thetac]
			fdata,ans[0,idx],ans[1,idx],ans[2,idx]=self.return_snr_lk(thetac,Tc,mask_fdata=mask_fdata)
			ans[3,idx]=self.fn_yerr_norm(thetac)
